...
function dirs_with_max_total_size(size, fs)
  local dirs = {}
...
function sum(paths, fs)
  local result = 0
    for i, path in pairs(paths) do
      result = result + fs[path]["size"]
    end
  return result
end
...
print(sum(dirs_with_max_total_size(tonumber(arg[2]), fs), fs))
